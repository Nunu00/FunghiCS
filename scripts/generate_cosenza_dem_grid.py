...
def get_cosenza_elevation(lat, lon):
    # 1. Mare Tirreno (Lon < 15.95 e lat 39.0..40.2)
    if lon < 15.95:
        if lon < 15.92:
            return 0.0, 0.0, "N"
        else:
            alt = max(0.0, (lon - 15.91) * 2200.0)
            return alt, 5.0, "W"
            
    # 2. Mare Jonio (Lon > 16.58 al nord, > 16.68 al centro-sud)
    if (lat > 39.60 and lon > 16.58) or (lat <= 39.60 and lon > 16.68):
        return 0.0, 0.0, "E"
    ...
    if 39.18 <= lat <= 39.65 and 16.16 <= lon <= 16.32:
        ...
        alt = max(150.0, 450.0 - (dist_asse * 2800.0))
        return alt, 4.0, "S"
    ...
    if 39.82 <= lat <= 40.15 and 16.02 <= lon <= 16.38:
        ...
        d2 = math.hypot(lat - 39.97, lon - 16.08)
        min_d = min(d1, d2)
        if min_d < 0.16:
